...
public class ArtMethodV22_64Bit extends ArtMethod {

    @FieldMapping(offset = 20)
    private Field access_flags_;

    @FieldMapping(offset = 40, length = 8)
    private Field entry_point_from_interpreter_;

    @FieldMapping(offset = 48, length = 8)
    private Field entry_point_from_jni_;

    @FieldMapping(offset = 56, length = 8)
    private Field entry_point_from_quick_compiled_code_;

    ArtMethodV22_64Bit(Method method) {
        super(method);
    }
    ...
    public int getAccessFlags() {
        return (int) access_flags_.readInt();
    }
